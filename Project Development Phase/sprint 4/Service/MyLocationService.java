package com.example.containmentzonealerting.Service;
...
import com.example.containmentzonealerting.MainActivity;
...
public class MyLocationService extends BroadcastReceiver {
    public static final String ACTION_PROCESS_UPDATE = "com.example.containmentzonealerting.Service.UPDATE_LOCATION";
...
//                        postDataUsingVolley(Double.toString(location.getLatitude()),Double.toString(location.getLongitude()),context);
    ...
    private void getDataUsingVolley(Context context,Location location) {
        ...
        String url = "http://192.168.193.227:5000/location_data";
                    ...
                    public void onResponse(JSONArray response) {
                        for (int locationIndex = 0; locationIndex < response.length(); locationIndex++) {
                            try {
                                ...
                                l.setLatitude(new Double(j.getString("LAT")));
                                l.setLongitude(new Double(j.getString("LON")));
                                int locationId = j.getInt("SNO");
    ...
    private void postDataUsingVolley(String lat,String lon,Context context) {
        final RequestQueue queue = Volley.newRequestQueue(context);
        String url = "http://192.168.193.227:5000/post_user_location_data";
        SharedPreferences sharedPreferences = context.getSharedPreferences("user_data", 0);
    ...
    private void sendMailUsingVolley(Context context,int locationId) {
        final RequestQueue queue = Volley.newRequestQueue(context);
        String url = "http://192.168.193.227:5000/send_trigger";
        SharedPreferences sharedPreferences = context.getSharedPreferences("user_data", 0);
